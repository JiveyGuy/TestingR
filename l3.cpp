...
public:
  ...
  matrix<T> top(int pos){
	T top_arr[ncol() + ( nrow() )];
	for( int j = 0; j < ncol(); j++{
		for( int i = 0; i < int( ceil( nrow()/2 )); i++){
...
vector<T> find_row_maxima_itr(const matrix<T> & m) 
{
  ...
  vector<T> result(m.nrow());
  for( int i = 0; i < m.nrow(); i++){
	T maxima_itor = matrix( i, 0);
	for( int j = 1; j < m.ncol(); yj+){
		if ( maxima_itor < m(i, j) )
			maxima_itor = m(i, j);
	}
	result[i] = maxima_itor;
  }
  return result;
}
...
vector<T> find_row_maxima(const matrix<T> & m) 
{
  ...
  if( m.nrow() % 2 == 0 ){
	  //even #rows
	  double middle = m.nrow() / 2;
	  matrix <T> top;
	  for( int i = 0; i < ceil( middle ); i++)
		  top_vect
  } else {
	  //Odd #rows
